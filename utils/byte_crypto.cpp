...
std::default_random_engine byte_crypto::random;
...
uint8_t* byte_crypto::get_hmac_sha1(const std::string& key, const uint8_t* data, size_t len) {
    int ret = 0;

    ret = HMAC_Init_ex(byte_crypto::hmac_sha1_ctx, key.c_str(), key.length(), EVP_sha1(), nullptr);
    if (ret != 1) {
        MS_THROW_ERROR("OpenSSL HMAC_Init_ex() failed with key '%s'", key.c_str());
    }

    ret = HMAC_Update(byte_crypto::hmac_sha1_ctx, data, static_cast<int>(len));
    if (ret != 1) {
        MS_THROW_ERROR("OpenSSL HMAC_Update() failed with key '%s' and data length %zu bytes", key.c_str(), len);
    }
