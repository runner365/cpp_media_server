...
dts:%ld, pts:%lu, key:%d, seq:%d, data len:%lu",
    ...
    if (pkt_ptr->av_type_ == MEDIA_VIDEO_TYPE) {
        on_video_packet_callback(pkt_ptr);
    } else if (pkt_ptr->av_type_ == MEDIA_AUDIO_TYPE) {
        on_audio_packet_callback(pkt_ptr);
    } else {
        log_errorf("unkown av type:%d", pkt_ptr->av_type_);
    }
}

void rtmp_player::on_video_packet_callback(MEDIA_PACKET_PTR pkt_ptr) {
...
void rtmp_player::on_audio_packet_callback(MEDIA_PACKET_PTR pkt_ptr) {
    pkt_ptr->buffer_ptr_->consume_data(2);
...
void rtmp_player::on_avframe_callback(int stream_index, AVFrame* frame) {
...
void rtmp_player::handle_video_avframe(AVFrame* frame) {
...
*/
void rtmp_player::handle_audio_avframe(AVFrame* frame) {
    int ret;
...
void rtmp_player::handle_video_extra_data(uint8_t* extra_data, size_t extra_len) {
    uint8_t sps[1024];
    uint8_t pps[1024];
    size_t sps_len = 0;
    size_t pps_len = 0;
...
void rtmp_player::updata_sps(uint8_t* sps, size_t sps_len) {
    if (sps_.empty()) {
...
void rtmp_player::updata_pps(uint8_t* pps, size_t pps_len) {
    if (pps_.empty()) {
